	...
	<div>
		<form id="form-insert">
			    ...
			    <div class="col-sm-10">
			    	<select id="categoryIdc" class="form-control form-control-sm" required>
					  ...
					  <c:forEach var="category" items="${categoryListByParentId}">
					  	<option class="option ${category.id}" value="${category.id}">${category.name}</option>
					  </c:forEach>
					</select>
			    </div>
			</div>
			    ...
			    <div class="col-sm-10">
			    	<select id="categoryId" name="categoryId" class="form-control form-control-sm" required>
					  ...
					  <c:forEach var="category" items="${categoryListByChildId}">
					  	<option class="option ${category.id}" value="${category.parentId}">${category.name}</option>
					  </c:forEach>
					</select>
			    </div>
			</div>
		  ...
		  <div class="form-group row">
		    ...
		    <div class="col-sm-10">
		      <input type="text" class="form-control" id="name" name="name" placeholder="Tên sản phẩm" required="required" >
		    </div>
		  </div>
		  ...
		  <div class="form-group row">
		    ...
		    <div class="col-sm-10">
		      <input type="text" class="form-control" id="quantity" name="quantity" placeholder="Số lượng" required="required">
		    </div>
		  </div>
		  ...
		  <div class="form-group row">
		    ...
		    <div class="col-sm-10">
		      <input type="text" class="form-control" id="Đơn giá" name="price" placeholder="Đơn giá" required="required">
		    </div>
		  </div>
		  ...
		  <div class="form-group row">
		    ...
		    <div class="col-sm-10">
		      ...
		      <input type="text" class="form-control mt-2" id="image_input" name="image" placeholder="Hình ảnh" required="required">
		    </div>
		  </div>
...
<script>
	...
	/* ------------------------------------phuong thuc them data------------------------------------- */
	
	
	async function addData(data){
		await fetch('http://localhost:8080/admin/product/add-handler', {
			method: 'POST',
			headers:{
	...
	btnAdd.addEventListener('click', (e)=>{
		...
		console.log(data);
		addData(data);
		res = confirm('Bạn thêm mới sản phẩm thành công ^^')
